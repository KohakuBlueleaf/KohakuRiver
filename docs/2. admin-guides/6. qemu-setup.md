...
`libguestfs-tools` provides `virt-customize`, which is used by `kohakuriver qemu image create` to customize base images (install packages, configure SSH, install NVIDIA drivers). Without it, images will be minimal and require manual setup.
...
# Image customization tool (for kohakuriver qemu image create)
...
## Installing NVIDIA Drivers Inside a Running VM

If the base image was created without `--with-nvidia`, or you need to install/update drivers inside an already-running VM, follow these steps from within the guest.
...
### "virt-customize: not found (image will be minimal)"

The `virt-customize` tool from `libguestfs-tools` is required for full image customization (installing packages, SSH setup, NVIDIA drivers). Without it, `kohakuriver qemu image create` produces a raw cloud image that only has the default cloud-init setup.
...
After installing, re-run `kohakuriver qemu image create` to get a fully customized image.
