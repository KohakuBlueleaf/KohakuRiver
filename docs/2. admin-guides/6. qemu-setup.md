...
## Step 4: Disable nvidia_drm Kernel Modesetting (for GPU Passthrough)

> **Required for dynamic GPU passthrough on consumer NVIDIA cards (e.g., RTX 2080 Ti, RTX 3090, RTX 4090).** Skip this step if your GPUs are bound to vfio-pci at boot (see Step 5 alternative) or if you only use datacenter GPUs (Tesla, A100, H100) which are unaffected.

`nvidia_drm` is the NVIDIA kernel modesetting module — it manages display output. When loaded with `modeset=1` (the default on many desktop distros), it holds references to **all** NVIDIA GPUs on the system, which makes runtime unbinding from the `nvidia` driver hang indefinitely. This blocks GPU passthrough.

On headless compute nodes (e.g., using AST2500/IPMI for console), `nvidia_drm` is unnecessary since no GPU drives a display.

### Option A: Disable Modesetting (Recommended for Headless Nodes)
...
Add `nvidia_drm.modeset=0` to `GRUB_CMDLINE_LINUX_DEFAULT`:

```bash
# Intel + IOMMU + no nvidia modesetting
GRUB_CMDLINE_LINUX_DEFAULT="quiet splash intel_iommu=on iommu=pt nvidia_drm.modeset=0"

# AMD + IOMMU + no nvidia modesetting
GRUB_CMDLINE_LINUX_DEFAULT="quiet splash amd_iommu=on iommu=pt nvidia_drm.modeset=0"
```
...
### Option B: Blacklist nvidia_drm Entirely

If you never need display output from NVIDIA GPUs on this node:
...
```bash
# Check modeset is disabled (should show "N" or module not loaded)
cat /sys/module/nvidia_drm/parameters/modeset 2>/dev/null || echo "nvidia_drm not loaded"

# Verify nvidia driver still works for compute
nvidia-smi
```

> **Why this matters:** With `modeset=1`, writing to `/sys/bus/pci/devices/<GPU>/driver/unbind` blocks indefinitely (the kernel thread never returns). The driver symlink may disappear, making it look like the unbind succeeded, but all subsequent sysfs operations on that device also hang — including `driver_override` writes and rebinding. A reboot is the only recovery. With `modeset=0` or `nvidia_drm` unloaded, unbinding completes normally.

---

## Step 5: Set Up VFIO (for GPU Passthrough)
...
### GPU unbind hangs / VM creation stuck at "Unbinding from nvidia"

This is caused by `nvidia_drm.modeset=1` holding references to all GPUs. See **Step 4** to disable it.

If the system is already stuck (sysfs writes hanging), the only recovery is a reboot. After rebooting, apply the Step 4 fix before trying again.

To recover stuck VFIO GPUs after a reboot without the fix yet applied:

```bash
sudo bash scripts/recover-vfio.sh
```
